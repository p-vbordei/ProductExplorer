...
try:
    from src import app
    from src.reviews_data_processing_utils import process_datapoints, quantify_observations, generate_batches, add_uid_to_reviews, aggregate_all_categories, attach_tags_to_reviews, quantify_category_data
    from src.firebase_utils import initialize_firestore, get_clean_reviews , write_reviews_to_firestore, write_insights_to_firestore, get_product_data_from_investigation
    ...
    from reviews_data_processing_utils import process_datapoints, quantify_observations, generate_batches, add_uid_to_reviews, aggregate_all_categories, attach_tags_to_reviews, quantify_category_data
    from firebase_utils import initialize_firestore, get_clean_reviews , write_reviews_to_firestore, write_insights_to_firestore, get_product_data_from_investigation
...
def run_reviews_investigation(investigationId: str) -> None:
    ...
    if not  tagedReviews, quantifiedDataId:
        logging.error("Error processing reviews with GPT.")
        return
    
    # Write quantified data to Firestore
...
#############
    ...
    if not update_investigation_status(investigationId, 'finishedProcessingReviewsWithGPT', db):
        logging.error(f"Error updating investigation status to 'finishedProcessingReviewsWithGPT'.")
        return



    if not write_insights_to_firestore(investigationId, quantifiedDataId, db):
        logging.error("Error writing insights to Firestore.")
        return
